...
def config_harness_name_should_be_standard(path):
    for fn in walk(path):
        ...
        with open(fn, mode="r", encoding="utf-8") as f:
            ...
            if config.get('BM1684X') != None and config['BM1684X'].get('harness') != None:
                harness_args_dict = config['BM1684X'].get('harness')
                for k, v in harness_args_dict.items():
                    if k == 'args':
                        if len(v) == 1:
                            break
                        name_list = []
                        name_list.append(v[0].get('name'))
                        for iter in range(1, len(v)):
                            if v[iter].get('name') not in name_list:
                                name_list.append(v[iter].get('name'))
                                continue
                            else:
                                name_list.append(v[iter].get('name'))
                                assert name_list, f'harness name should be different'
